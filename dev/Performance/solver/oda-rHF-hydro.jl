...
@timeit to "Init Solver" begin
    ...
    @timeit to "Init Cache discretization" init_cache!(
        discretization, model, true, ExactIntegration())
    ...
    solver = KohnShamSolver(niter, stopping_criteria, discretization,
        model, method, cache, opts, energies, logbook)
end
