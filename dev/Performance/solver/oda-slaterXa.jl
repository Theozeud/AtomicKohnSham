...
@timeit to "Init Solver" solver = KohnShamResolution.init(model, discretization, method; scftol = 1e-3, hartree = false, logconfig = LogConfig(orbitals_energy = true))
        ...
        if !cache.flag_degen
            ...
            @timeit to "compute energy" begin 
                ...
                @timeit to "Eexc" begin 
                    ...
                    @timeit to "Eexc" energies[:Eexc] = compute_exchangecorrelation_energy(discretization, model, D)
                end
            end
        end
