...
function eigen_with_finite_diff(Nmesh, Rmax, T)
    A = SymTridiagonal(2*ones(T, Nmesh), - ones(T, Nmesh-1)) .*
        ((T(Nmesh) - T(1)) / T(Rmax))^2
    eigvals(A)
end
...
function plot_eigenvector(Nmesh, Rmax, n, order)
    ...
    plot!(plt_eigenvector, X, (sign(vect_theo(n, Rmax)(1) * eigs_il(1)) * eigs_il).(X),
        label = "P1 + Integrated Legendre ordre "*string(order), lw = 4)
...
function compute_error_eigenvalue(vecNmesh, Rmax, num, T)
    ...
    for i in eachindex(label)
        plot!(plt_ϵ_error, vecNmesh, ϵerror[:, i], lw = 4,
