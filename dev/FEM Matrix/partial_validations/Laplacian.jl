...
struct LaplacianProblem
    ...
    function LaplacianProblem(;
            name, T, typebasis, optsbasis, typemesh, optsmesh, Rmax, Nmesh)
        new(name, T, typebasis, optsbasis, typemesh, optsmesh, Rmax, Nmesh)
    end

    function LaplacianProblem(
            prob; name = prob.name, T = prob.T, typebasis = prob.typebasis,
...
struct LaplacianConvergenceNmesh
    ...
    nums::Any            # Number of eigenvalue and eigenvector used 
end
...
function plot_eigenvalue(sols)
    ...
    plot!(plt, theoretical_eigenvalue.(1:mindex, RMAX, Float64),
        label = "Theoretical", lw = 4, ls = :dash, lc = :black)
    return plt
end
...
function plot_eigenvector(sols, n; resol = 5000)
    ...
    for sol in sols
        ...
        plot!(plt,
            X,
            (sign(theoretical_eigenvector(n, Rmax, sol.prob.T)(1) * eign_n(1)) * eign_n).(X),
            label = sol.prob.name,
            lw = 4)
        if sol.prob.Rmax > RMAX
            RMAX = sol.prob.Rmax
    ...
    plot!(plt, X, theoretical_eigenvector(n, Rmax, sol.prob.T).(X),
        label = "Theoretical n = $n", lw = 4)
    plt
end
...
function convergence_plot_Nmesh(sols::LaplacianConvergenceNmesh)
    ...
    for prob in sols.probs
        for num in sols.nums
            plot!(plt, sols.vecNmesh, sols.ΔΛ[prob.name][:, num], lw = 4,
