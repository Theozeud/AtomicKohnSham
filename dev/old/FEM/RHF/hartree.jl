...
function approx_intH10(mesh, ρ, T)
    ...
    vect[1] = KohnShamResolution.approximate_integral(
        f, (Rmin, Rmax); method = QuadGKJL(), reltol = 100*eps(T), abstol = 100*eps(T)) *
              Rmin
    for i in eachindex(vect)[2:end]
        domain1 = (Rmin, T(mesh[i]))
        vect[i] = KohnShamResolution.approximate_integral(
            g, domain1; method = QuadGKJL(), reltol = 100*eps(T), abstol = 100*eps(T))
        domain2 = (T(mesh[i]), Rmax)
        vect[i] += mesh[i] * KohnShamResolution.approximate_integral(
            f, domain2; method = QuadGKJL(), reltol = 100*eps(T), abstol = 100*eps(T))
    end
...
function approx_int(mesh, ρ, T)
    ...
    for i in eachindex(vect)[2:end]
        ...
        vect[i] = 1/mesh[i] * KohnShamResolution.approximate_integral(
            g, domain1; method = QuadGKJL(), reltol = 100*eps(T), abstol = 100*eps(T))
        domain2 = (mesh[i], Rmax)
        vect[i] += KohnShamResolution.approximate_integral(
            f, domain2; method = QuadGKJL(), reltol = 100*eps(T), abstol = 100*eps(T))
...
function potential_intlegH10(mesh, ordermax, ρ, T)
    basis = ShortP1IntLegendreBasis(mesh, T; ordermin = 2, ordermax = ordermax,
        normalize = true, left = false, right = false)
    potentialH10(basis, ρ)
end
