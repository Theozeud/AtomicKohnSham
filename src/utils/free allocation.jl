...
function mul_block_by_block!(
        Γout::BlockDiagonal{<:Number}, M::AbstractMatrix, Γin::BlockDiagonal{<:Number})
    if nbblock(Γout) ≠ nbblock(Γin)
        throw(DimensionMismatch(lazy"Γout has $(nbblock(Γout)) blocks but Γin has $(nbblock(Γin))"))
    end
    for i in 1:nbblock(Γout)
...
function mul_block_by_block!(
        Γout::BlockDiagonal{<:Number}, Γin::BlockDiagonal{<:Number}, M::AbstractMatrix)
    if nbblock(Γout) ≠ nbblock(Γin)
        throw(DimensionMismatch(lazy"Γout has $(nbblock(Γout)) blocks but Γin has $(nbblock(Γin))"))
    end
    for i in 1:nbblock(Γout)
...
function _copy_vec_to_mat!(
        M::AbstractMatrix, ir_dest::AbstractRange{Int}, jr_dest::AbstractRange{Int},
