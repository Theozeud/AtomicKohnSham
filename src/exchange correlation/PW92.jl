...
function exc(lsda::PW91, rhoUP::Real, rhoDOWN::Real)
    ex(lsda, rhoUP, rhoDOWN) + ec(lsda, rhoUP, rhoDOWN)
end
function vxcUP(lsda::PW91, rhoUP::Real, rhoDOWN::Real)
    vxUP(lsda, rhoUP) + vcUP(lsda, rhoUP, rhoDOWN)
end
function vxcDOWN(lsda::PW91, rhoUP::Real, rhoDOWN::Real)
    vxDOWN(lsda, rhoDOWN) + vcDOWN(lsda, rhoUP, rhoDOWN)
end
...
@fastmath function _vc(::PW91, rhoUP::Real, rhoDOWN::Real, σ::Int)
    ...
    ∂εcPW∂ξ = 4 * ξ^3 * fξ * (ΔεcPW10 + mαc * invd2f0) +
              derivf(ξ) * (ξ4 * ΔεcPW10 - mαc * c1)
...
@fastmath function G∂G(
        rs::Real, p::Real, A::Real, α₁::Real, β₁::Real, β₂::Real, β₃::Real, β₄::Real)
