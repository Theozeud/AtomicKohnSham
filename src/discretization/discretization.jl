...
struct KSEDiscretization{T <: Real, 
    ...
    function KSEDiscretization(lₕ::Int, basis::Basis, mesh::Mesh, nₕ::Int = length(basis))
        elT = eltype(basis)
        Nₕ = length(basis)
        new{eltype(mesh), elT, typeof(basis)}(lₕ, nₕ, Nₕ, basis, mesh, first(mesh), last(mesh), elT, create_cache_discretization(lₕ, Nₕ, elT)...)
    end
end
...
function zero_energies(kd::KSEDiscretization, model::KohnShamExtended)
    @unpack elT = kd
...
#--------------------------------------------------------------------
function prepare_eigenvalue_problem!(   discretization::KSEDiscretization, 
                                        model::KSEModel, 
