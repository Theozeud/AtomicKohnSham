...
"""
struct P1IntLegendreGenerator{T, polyType <: PolySet, derivpolyType <: PolySet} <:
       AbstractGenerator{T}
    ...
    function P1IntLegendreGenerator(
            T::Type = Float64; ordermax::Int = 2, binf::Real = -T(1), bsup::Real = T(1))
        @assert ordermax ≥ 1
...
function getderivpolynomial(p1ilg::P1IntLegendreGenerator, I::AbstractVector{Int})
...
function P1IntLegendreBasis(mesh::Mesh, T::Type = Float64; kwargs...)
    ...
    numbas += 1
    for j in 2:generators.ordermax
    ...
    # Mid-meshes
    for i in eachindex(mesh)[2:(end - 2)]
        indices_cells[numbas] = (i, i+1)
        indices_generators[numbas] = (1, 2)
        numbas += 1
        for j in 2:generators.ordermax
